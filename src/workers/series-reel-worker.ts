            ...
            while (attempts < maxAttempts) {
                ...
                try {
                  ...
                  const promptPrefix = isStylized 
                    ? `A professional 9:16 ${series.artStyle || 'stylized'} artwork of:`
                    : `A professional 9:16 cinematic photograph of:`;
                  
                  const cameraSettings = isStylized
                    ? `Beautiful color harmony, rich composition, high-end design.`
                    : `Cinematic lighting, professional photography, natural details, ${series.artStyle || 'cinematic'} aesthetic.`;
                    
                  const negPrompt = "text, watermarks, borders, signature, low quality, distorted, extra limbs, deformed hands";

                  const engineeredPrompt = JSON.stringify({
                    prompt: `${promptPrefix} ${keyword}. ${cameraSettings} Lighting: ${lighting || 'cinematic'}.`,
