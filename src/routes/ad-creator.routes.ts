...
- lighting: One exact lighting plan repeated on every slide (direction, quality, shadows, color temperature). ${refFiles.length > 0 ? 'CRITICAL: Extract and lock the exact lighting setup directly from the attached Reference Image(s).' : ''}
- colorPalette: The exact color palette (with hex codes) that appears on every slide. ${refFiles.length > 0 ? 'CRITICAL: Extract the color palette directly from the attached Reference Image(s).' : ''}
- typography: The exact headline + subheadline font styles, text color, text placement used on every slide. ${refFiles.length > 0 ? 'CRITICAL: Replicate the exact font style, font weight, letter spacing, color, and placement from the attached Reference Image(s).' : ''}
...
- Slide 2 ASPIRATIONAL LIFESTYLE IN-ACTION PHOTO: MUST be a dedicated lifestyle scene showing the garment in real-life action on a model in an authentic, relatable lifestyle context (e.g., walking through a sunlit courtyard, candid movement in a luxury lounge, or relaxed café setting). This lets buyers instantly visualize wearing the product in real life. Caption highlights movement, fit, and day-to-night versatility.
...
10. ASPIRATIONAL LIFESTYLE IN-ACTION PHOTO: If this is Slide 2 (or role 'lifestyle'), the imagePrompt MUST explicitly describe an aspirational lifestyle scene featuring a model wearing the product in real-life action/movement (e.g., walking through a sunlit courtyard, candid lounge movement, or ambient cafe setting) so buyers instantly picture themselves wearing the garment in real life.`;
