...
if __name__ == "__main__":
    ...
    run_scp("scratch/socialsched_vps.env", "aqil@187.127.154.55:/tmp/.env.socialsched")
    run_scp("scratch/socialsched-nginx.conf", "aqil@187.127.154.55:/tmp/socialsched-nginx.conf")
    
    commands = [
        "mv /tmp/.env.socialsched /etc/socialsched/.env",
        "chown root:socialsched-dev /etc/socialsched/.env",
        "chmod 640 /etc/socialsched/.env",
        "mv /tmp/socialsched-nginx.conf /etc/nginx/sites-available/socialsched.vibeship.in",
        "systemctl reload nginx",
        "systemctl restart socialsched-backend",
        "systemctl status socialsched-backend --no-pager"
    ]
